(function($,window){
  var Project = function(options){
    ...
    this.outPutToEmailUrl = "http://usuapp.com/project/sendProject";
  ...
  Project.prototype = {
    ...
    getAllElement: function(){
      ...
      this.outputEmailBtn = this.content.querySelector("#output-email");
    ...
    addEvent: function(){
      ...
      this.outputEmailBtn.addEventListener("click",function(event){
        self.outputEmail(event);
      });
      ...
      mui.prompt('请输入您要接收的邮箱：', '', '优速Max', btnArray, function(e) {
        if(e.index==0){
          ...
          if(userInfo){
            ...
            self.User.sendAjaxInfo.call(self,info,self.uploadForEmailResult,"json");
          }else{
     ...
     */
    uploadForEmailResult: function(data,self){
      if(data){
        switch(data.info){
          case "wrongToken":
    ...
    hasExist: function(itemList,key,value){
      for(var i=0,len=itemList.length;i<len;i++){
        if(itemList[i][key]==value) return {index:i};
      }
      return false;
    },
